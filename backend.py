...
def load_xgboost_model():
    ...
    if xgboost_model is None:
        xgboost_model = joblib.load('model_gia_nha_xgboost.pkl')
...
def load_and_clean_data():
    global lambdas
    data = pd.read_csv("clean_data_without_outliers_with_skewness.csv")
...
def get_clustering_model(df):
    ...
    df_cluster = df.dropna(subset=num_cols).copy()
    # Apply standardizations
    for col in num_cols:
        df_cluster[col] = df_cluster[col].fillna(df_cluster[col].median())
    
    X = df_cluster[num_cols].values
    pca = PCA(n_components=3, random_state=42)
    X_pca = pca.fit_transform(X)
    
    kmeans = KMeans(n_clusters=4, random_state=42, n_init=10)
    df_cluster['prediction'] = kmeans.fit_predict(X_pca)
    
    # Add back PCA coordinates for 3D visualization
    df_cluster['PC1'] = X_pca[:, 0]
    df_cluster['PC2'] = X_pca[:, 1]
    df_cluster['PC3'] = X_pca[:, 2]
    
    return kmeans, pca, df_cluster

@st.cache_resource
def get_anomaly_model(df):
    model_path = 'model_anomaly_detection_IsolationForest.pkl'
    stats_path = 'anomaly_stats.pkl'
